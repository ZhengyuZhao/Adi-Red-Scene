...
for dataset in datasets:
    image_path='./datasets/'+dataset+'/images/'
    label_path='./datasets/'+dataset+'/labels/'
            ...
            if selection_type=='adi_red':      
                for scale in scales:
                    ...
                    for mode in modes:
                        ...
                        if scale=='1':                        
                            np.save(result_path+'total_features_'+scale+'_'+mode+'_'+arch+'_'+pretrain_databases[int(scale)-1]+'_'+resolution+'.npy', total_features)
                            if dataset=='SUN397':
                                for split in splits:
                                    split_index=np.load(label_path+'index_'+mode+'_'+split+'.npy')                          
                                    np.save(result_path+'total_features_'+scale+'_'+mode+'_'+arch+'_'+pretrain_databases[int(scale)-1]+'_'+resolution+split+'.npy',total_features[split_index])
                        if scale=='2' or scale=='3':
                            for tt,T in enumerate(thresholds):
